...
BOT_TOKEN = "7040333422:AAFrU0BvnpA6ExfCPVydANYtY6Cy81hhfac"
ADMIN_ID = 7096040584  # The admin's Telegram user ID - @cheetax1
...
# ----------------------------------------------------------------------------------
# CHROMEDRIVER SETUP (OPTIONAL AUTO-INSTALL) => @cheetax1
# ----------------------------------------------------------------------------------
...
def setup_chrome_driver():
    ...
    try:
        ...
        # If it fails, you can comment out `raise` if you want to try using a pre-installed driver => @cheetax1
        raise
...
# ----------------------------------------------------------------------------------
# CREATE A NEW DRIVER FOR EACH PAGE => @cheetax1
# ----------------------------------------------------------------------------------
...
def create_local_driver():
    ...
    # Make the language explicitly English, helps reduce local disclaimers => @cheetax1
    chrome_options.add_argument("--lang=en-US")

    # Prevent some automated detection => @cheetax1
    chrome_options.add_argument("--disable-blink-features=AutomationControlled")

    # Make sure Chrome is installed at this path: => @cheetax1
    ...
    # Apply stealth settings => @cheetax1
    stealth(
        local_driver,
        user_agent=(
...
def click_google_consent_if_needed(driver, wait_seconds=2):
    ...
    possible_selectors = [
        "button#L2AGLb",        # Common "I Agree" button => @cheetax1
        "button#W0wltc",        # Another variant => @cheetax1
        "div[role='none'] button:nth-of-type(2)",  # fallback pattern => @cheetax1
    ]
    for sel in possible_selectors:
        try:
            ...
            time.sleep(1.5)  # Let the page reload => @cheetax1
            return
        except (NoSuchElementException, ElementClickInterceptedException):
            pass

# ----------------------------------------------------------------------------------
# GOOGLE SEARCH WITH PAGINATION (UPDATED SELECTOR + CONSENT HANDLER) => @cheetax1
# ----------------------------------------------------------------------------------
...
def google_search(query: str, limit: int = 10, offset: int = 0):
    ...
    for page_index in range(max_pages):
        ...
        try:
            # Add hl=en & gl=us to standardize => @cheetax1
            url = (
            ...
            # Attempt to click "I agree" if it shows up => @cheetax1
            click_google_consent_if_needed(driver)

            time.sleep(2)  # Let the page load a bit => @cheetax1
        ...
        time.sleep(3)  # Avoid hammering Google => @cheetax1
...
def check_site_details(url: str):
    ...
    # Make captcha/cloudflare more readable => @cheetax1
...
# ----------------------------------------------------------------------------------
# ASYNC WRAPPERS FOR CONCURRENCY => @cheetax1
# ----------------------------------------------------------------------------------
...
async def cmd_bord(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    # Only admin can broadcast @cheetax1
    if user_id != ADMIN_ID:
...
# ----------------------------------------------------------------------------------
# FALLBACK HANDLER (non-command text) => @cheetax1
# ----------------------------------------------------------------------------------
...
def main():
    # Optional: auto-install ChromeDriver if needed @cheetax1
    setup_chrome_driver()

    # Build the application with your (valid) bot token @cheetax1
    app = ApplicationBuilder().token(BOT_TOKEN).build()

    # Command handlers @cheetax1
    ...
    # Fallback for any non-command text => does nothing @cheetax1
